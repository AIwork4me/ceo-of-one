  ...
  return (
    <section className="min-h-screen flex items-center justify-center relative overflow-hidden pt-20 bg-gradient-to-br from-surface-dim to-surface">
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
        ...
        {/* M3 Display Medium */}
        <h1 className="text-[28px] sm:text-[36px] md:text-[45px] font-semibold text-on-surface mb-6 leading-[1.2]">
          {t('title')}
        </h1>
        {/* M3 Body Large */}
        <p className="text-[16px] sm:text-[18px] text-on-surface-variant mb-10 max-w-2xl mx-auto leading-[1.5]">
          {t('subtitle')}
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10 max-w-2xl mx-auto">
          <div className="rounded-m3-lg p-5 bg-surface-container border border-outline-variant">
            ...
            <div className="text-[14px] text-on-surface-variant leading-relaxed">{t('before.text')}</div>
          </div>
          <div className="rounded-m3-lg p-5 bg-surface-container border border-outline-variant">
            <div className="text-[12px] font-medium text-success uppercase tracking-wider mb-2">{t('after.label')}</div>
            <div className="text-[14px] text-on-surface-variant leading-relaxed">{t('after.text')}</div>
          </div>
        </div>
        ...
        <a
          href="#outline"
          className="inline-block bg-primary-container hover:bg-primary-container-high text-on-surface px-8 py-4 rounded-m3-md text-[14px] font-medium tracking-wide transition-all transform hover:scale-[1.02] shadow-m3-1"
        >
          {t('cta')}
        </a>
  ...
  return (
    <section className="py-20 bg-surface-dim">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* M3 Headline Large */}
        <h2 className="text-[28px] sm:text-[32px] font-semibold text-on-surface text-center mb-12">
          {t('title')}
        </h2>
  ...
  return (
    <section className="py-24 bg-surface">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-[28px] sm:text-[32px] font-semibold text-on-surface text-center mb-16">
          {t('title')}
        </h2>
          ...
          {steps.map((step, index) => (
            <div key={step.key} className="flex items-center">
              <div className="bg-surface-dim rounded-m3-lg p-6 text-center min-w-[160px] border border-outline-variant/50">
                ...
                {/* M3 Title Large */}
                <div className="text-[22px] font-medium text-on-surface mb-1">{step.title}</div>
                {/* M3 Body Small */}
        ...
        <div className="mt-12 text-center">
          <div className="inline-flex items-center gap-3 bg-surface-dim rounded-m3-lg px-8 py-4 border border-primary/30">
            ...
            <span className="text-[16px] font-medium text-on-surface">{t('result.title')}</span>
          </div>
        </div>
      </div>
    </section>
        ...
        <div className="space-y-3">
          {chapters.map((chapter) => (
            ...
            >
              ...
              <div>
                <h3 className="text-[16px] font-medium text-on-surface">{chapter.title}</h3>
          ...
          {testimonials.map((item, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-[14px] font-medium text-on-surface">{item.author}</div>
        ...
        <div className="bg-surface rounded-m3-xl p-8 border border-outline-variant/50 shadow-m3-2">
          <div className="text-center mb-8">
            ...
            <div className="flex items-baseline justify-center gap-2">
              <span className="text-[36px] font-semibold text-on-surface">{t('price')}</span>
            ...
            <div className="mt-4 space-y-2 max-h-64 overflow-y-auto pr-2">
              {chapters.map((ch) => (
                <div key={ch.num} className="flex items-start gap-3 text-[14px]">
                  ...
                  <div>
                    <span className="text-on-surface font-medium">{ch.title}</span>
          ...
          <button className="w-full bg-primary-container hover:bg-primary-container-high text-on-surface py-4 rounded-m3-md font-medium text-[14px] tracking-wide transition-all transform hover:scale-[1.02] shadow-m3-1 mt-4">
            {t('cta')}
          </button>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {faqs.map((faq, index) => (
            <div key={index} className="bg-surface-dim rounded-m3-md border border-outline-variant/50 overflow-hidden">
              ...
              >
                <span className="text-[16px] font-medium text-on-surface">{faq.q}</span>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            <a
              ...
              className="text-[14px] text-onsurface-variant hover:text-on-surface transition-colors"
            >
              GitHub
            </a>
          </div>
